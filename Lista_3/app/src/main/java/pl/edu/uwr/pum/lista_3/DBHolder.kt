...
import pl.edu.uwr.pum.lista_3.Table

class DBHolder(context: Context) : SQLiteOpenHelper(
    ...
    fun getElement(num: Int): List<Table> {
        val endlist: MutableList<Table> = ArrayList()
    ...
    fun getElementtt(num: Int): List<Table> {
        val endlist: MutableList<Table> = ArrayList()
        ...
        if(endlist.isEmpty()){
            var emptyL:List<Table> = listOf(Table(0,0,"none",0, 0))
            return emptyL
        }else {
            return endlist
        }
    ...
    fun addToGallery(singleItem: Table): Long{
        val db = this.writableDatabase
